...
#' @param output_type c("html", "pdf", "word", "Rmd", "none"). Creates a file called 'design_comparison.html' unless file_prefix is provided (or a different file type is specified). 'Rmd' means write R markdown only (otherwise Rmd is saved along with desired type of output file).
...
print_dc <- function(dc, display, file_prefix = "design_comparison", output_type = "html", to_Rmd = FALSE){
  ...
  if(output_type != "none" && !to_Rmd){
    ...
    if(output_type == "word")
      header <- header[-6] # check for alternate word syntax
